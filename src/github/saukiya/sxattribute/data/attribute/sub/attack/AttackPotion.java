...
public class AttackPotion extends SubAttribute {
    ...
    public void onEnable() {
        ...
        for (String name : config().getConfigurationSection("List").getKeys(false)) {
            String typeName = config().getString("List." + name + ".Type");
    ...
    public void onReLoad() {
        for (String typeName : config().getConfigurationSection("List").getKeys(false)) {
    ...
    public class PotionData {
        ...
        public void load() {
            ...
            this.combatPower = config().getInt("List." + name + ".CombatPower", 1);
            this.upperLimit = config().getInt("List." + name + ".UpperLimit", 100);
            this.messageName = getString("List." + name + ".MessageName");
            this.minLevel = config().getInt("List." + name + ".MinLevel", 1);
            this.maxLevel = config().getInt("List." + name + ".MaxLevel", 2);
            this.minTime = config().getInt("List." + name + ".MinTime", 2);
            this.maxTime = config().getInt("List." + name + ".MaxTime", 5);
        }
