...
public class OnHealthChangeListener extends BukkitRunnable implements Listener {
    ...
    public void run() {
        ...
        for (int i = nameList.size() - 1; i >= 0; i--) {
            ...
            if (nameData.getEntity() == null || nameData.getEntity().isDead() || nameData.getEntity().getHealth() == Sx.getMaxHealth(nameData.getEntity()) || nameData.getClearTime() < System.currentTimeMillis()) {
    ...
    void OnEntityDamageEvent(EntityDamageEvent event) {
        ...
        double maxHealth = Math.max(Sx.getMaxHealth(entity), 0);
        ...
        if (damager == null && Config.isHolographic() && Config.isHolographicHealthTake() && SXAttribute.isHolographic() && !Config.getHolographicBlackList().contains(event.getCause().name())) {
            ...
            holoData.getHologram().appendTextLine(Message.getMsg(Message.PLAYER__HOLOGRAPHIC__TAKE, event.getFinalDamage()));
    ...
    public String getHealthProgressName(double progress) {
        ...
        return healthName.append(Config.getConfig().getString(Config.HEALTH_NAME_VISIBLE_SUFFIX)).toString().replace("&", "§");
    }
