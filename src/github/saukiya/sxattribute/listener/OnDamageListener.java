...
public class OnDamageListener implements Listener {
    ...
    void onEntityDamageByEntityEvent(EntityDamageByEntityEvent event) {
        ...
        if (event.getDamager() instanceof Projectile && ((Projectile) event.getDamager()).getShooter() instanceof LivingEntity) {
            ...
            attackData = plugin.getApi().getProjectileData(event.getDamager().getUniqueId(), true);
        ...
        String entityName = plugin.getOnHealthChangeDisplayListener().getEntityName(defenseEntity);
        String damagerName = plugin.getOnHealthChangeDisplayListener().getEntityName(attackEntity);
        ...
        for (SubAttribute attribute : SXAttributeManager.getSubAttributes()) {
            ...
            } else if (attribute.containsType(SXAttributeType.DAMAGE)) {
                attribute.eventMethod(null, damageData);
            }

            if (damageData.isCancelled() || damageData.getDamage() <= 0) {
                event.setCancelled(damageData.isCancelled());
                damageData.setDamage(0.01D);
                break;
            }
        }
    }
