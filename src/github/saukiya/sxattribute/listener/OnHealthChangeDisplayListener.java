...
public class OnHealthChangeDisplayListener implements Listener {
    ...
    @Getter
    private final List<Hologram> hologramsList = SXAttribute.isHolographic() ? new ArrayList<>() : null;
    ...
    public OnHealthChangeDisplayListener(SXAttribute plugin) {
        this.plugin = plugin;
        this.mobManager = SXAttribute.isMythicMobs() ? MythicMobs.inst().getMobManager() : null;
        new BukkitRunnable() {
            @Override
            public void run() {
                //TODO 架构修改为List
    ...
    public void onDisable() {
        if (Config.isHealthBossBar() && getBossList().size() > 0) {
            for (BossBarData bossBarData : getBossList()) {
                bossBarData.getBossBar().removeAll();
            }
        }
        if (Config.isHealthNameVisible() && getNameList().size() > 0) {
        ...
        if (Config.isHolographicHealthTake() && getHologramsList().size() > 0) {
            for (Hologram hologram : getHologramsList()) {
                hologram.delete();
            }
        }
    }
    ...
    void OnEntityDamageEvent(EntityDamageEvent event) {
        ...
        if (event instanceof EntityDamageByEntityEvent) {
            ...
            if (damager != null) {
                // BossBar
                if (Config.isHealthBossBar() && damager instanceof Player && SXAttribute.getVersionSplit()[1] >= 9 && !Config.getBossBarBlackCauseList().contains(event.getCause().name()) && !isUseMMBossBar(entity)) {
                    if (bossBarData == null) {
        ...
        if (damager == null && Config.isHolographic() && Config.isHolographicHealthTake() && SXAttribute.isHolographic() && !Config.getHolographicBlackList().contains(event.getCause().name())) {
            ...
            getHologramsList().add(hologram);
            new BukkitRunnable() {
                @Override
                public void run() {
                    hologram.delete();
                    getHologramsList().remove(hologram);
                }
    ...
    public boolean isUseMMBossBar(LivingEntity entity) {
        if (SXAttribute.isMythicMobs()) {
            ActiveMob activeMob;
            return (activeMob = mobManager.getMythicMobInstance(entity)) != null && activeMob.getType().usesBossBar();
        }
        return false;
    }
