...
public enum Message {
    ...
    public static void loadMessage() throws IOException, InvalidConfigurationException {
        messages = new YamlConfiguration();
        if (!FILE.exists()) {
            Bukkit.getConsoleSender().sendMessage(getMessagePrefix() + "§cCreate Message.yml");
            createDefaultMessage();
            messages.save(FILE);
        } else {
            messages.load(FILE);
            if (detectionVersion()) {
                Bukkit.getConsoleSender().sendMessage(getMessagePrefix() + "§eUpdate Message.yml");
                messages.save(FILE);
            } else {
                Bukkit.getConsoleSender().sendMessage(getMessagePrefix() + "Find Message.yml");
            }
        }
